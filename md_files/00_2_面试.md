...
```mysql
#flink大状态存储
是否首次问题,存hbase,为了减少hbase访问次数,本地布隆过滤器存能大大减少内存占用。
存本地hashmap或者redis都不合适
...
方案1:每个查hbase，异步查询
方案2：每个查hbase，异步查询+本地状态布隆过滤器查询(是否存在场景)
			每个查hbase，异步查询+redis做热度维表缓存(获取维度数据场景,poi表等ßß)
			
4.布隆过滤器首次导入问题
把订单表从最早采集到kafka,然后过滤创建时间大于当前天。
...
按用户id取模并行去，然后本地缓存，布隆过滤器，并且开启异步查询。
...
通过flinkUI看内存使用率合理分配内存,
禁用算子链串行，然后看各个算子的反压情况，如果有反压增大并行度
...
```mysql
#2个选择
小时级调度，维度表的分区要变,订单表的分区要变24h，感觉不合适，就用实时处理几个主表
实时调度，这里有个仔细的问题，hbase用日期的二级索引需要用，但是hive的分区在hbase表没法使用,
而hivesql也无法使用phoenix创建的二级索引，只能用sparksql,通过phoenix索引查数据，然后用spark处理，写hbase
...
场景:默认字段如非商品中心商品的spuid都是0，这种要处理。


#通过血缘关系找到重复中间表进行合并，制定临时表建表规范(好复杂)
元数据表，记录表的血缘表,统计口径(group by ,full,filter),人工来辅助来做的,atlas不支持查看统计口径。
...
```mysql
#sql优化
热点问题：核销表关联poi表，获取每个区核销数据,城市核销数据。poi维度表是个大表
...
3 热点key join场景下,先对key采样，只对热点key进行打散，然后union 非热点key的正常join
给key+｜随机数(1-10),然后维度表去笛卡尔积1-10扩大10倍。这个场景是维度表数量级远小于主表,但是维度表也相对	大，不能mapjoin加载到内存
