...
#自定义函数用法奇怪
这里用了个自定义函数udf_time_sys(a2_v.mobile_tel, 'yyyyMMddHHmmss') as done_time_d
这两步是干什么的呢？？？
...
```mysql
数据源：6表
6,7任务完全和2，3相同,只是分别插入不同的表中
8任务和4任务相同,就是把视频和非视频的数据放在一起去重

4任务完事之后写入e_event_flow_stream
8任务完事之后也是写入e_event_flow_stream
